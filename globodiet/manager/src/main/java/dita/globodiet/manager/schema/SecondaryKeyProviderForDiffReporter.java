...
implements TabularData.SecondaryKeyProvider {
	...
	@Override
	public Optional<BitSet> lookupSecondaryKey(final String tableKey) {
		return Optional.ofNullable(lookup.get(tableKey));
	}

	private static BitSet bitSet(final int ... indices) {
    	var bitSet = new BitSet(8);
    	IntStream.of(indices).forEach(bitSet::set);
    	return bitSet;
    }
