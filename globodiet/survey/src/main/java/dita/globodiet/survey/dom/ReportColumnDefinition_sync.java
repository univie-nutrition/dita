...
public class ReportColumnDefinition_sync {
    ...
    private String extractKey(final String line) {
        int p = line.indexOf("]");
        if(p>0) return line.substring(0, p+1);
    ...
    private String key(final FoodComponent foodComponent) {
        return foodComponent.componentId().toString();
    }
    private String description(final FoodComponent foodComponent) {
        //TODO[dita-globodiet-survey-33] extract names from (SemanticIdentifierSet) attributes
        return String.format("%s unit=%s%s",
                "unnamed",
                Optional.ofNullable(foodComponent.metricPrefix()).map(MetricPrefix::getSymbol).orElse(""),
