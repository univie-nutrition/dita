...
import dita.globodiet.survey.dom.BlobStoreClient.DataSourceLocation;
...
public class Survey_uploadCorrection {
    ...
    public Survey act(
        ...
        if(interviewFileOrFiles.isZipped()) {
            interviewFileOrFiles.unzipAsBlobStream(CommonMimeType.YAML)
                .forEach(blob->uploadToBlobStore(client, blob));
        } else if(interviewFileOrFiles.isYaml()) {
            uploadToBlobStore(client, interviewFileOrFiles);
        } else {
    ...
    private void uploadToBlobStore(final BlobStoreClient client, final Blob blob) {
        ...
        var blobDescriptor = new BlobDescriptor(
                DataSourceLocation.CORRECTIONS.namedPath(client.surveyPath()).add(blob.name()),
                CommonMimeType.YAML,
                createdBy,
                Instant.now(),
                blob.bytes().length,
                ...
                Map.of("uncompressed-size", "" + blob.bytes().length,
                        "sha256", sha));
        surveyBlobStore.putBlob(blobDescriptor, blob);
    }
