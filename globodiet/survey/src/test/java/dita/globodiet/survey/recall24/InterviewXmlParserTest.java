...
""";
        ...
        var dto = JaxbUtils.tryRead(_Dtos.Itv.class, xml)
                .valueAsNonNullElseFail();

        System.err.printf("%s%n", JsonUtils.toStringUtf8(dto, JsonUtils::indentedOutput));

        //parser.parse(DataSource.ofStringUtf8(xml));
    ...
    void aggregation() {
        ...
        .ifPresent(sampler->{
            ...
            interviewSources.forEach(zippedDs->{
                ...
                var xml = new String(ds.bytes());

                System.err.printf("md5 %s%n", ds.md5Hex());

                System.err.printf("%s%n", new String(ds.bytes()));

                InterviewSetDto iSet = parser.parse(ds);

            });

//            var interviews = interviewSources.stream()
//                .peek(i->{
//                    System.err.printf("%s%n", i.md5Hex());
//                })
//                //.map(parser::tryParse)
