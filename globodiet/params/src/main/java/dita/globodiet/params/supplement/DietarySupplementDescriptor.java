/*
 * Licensed to the Apache Software Foundation (ASF) under one
 * or more contributor license agreements.  See the NOTICE file
 * distributed with this work for additional information
 * regarding copyright ownership.  The ASF licenses this file
 * to you under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance
 * with the License.  You may obtain a copy of the License at
 * 
 *       http://www.apache.org/licenses/LICENSE-2.0
 * 
 * Unless required by applicable law or agreed to in writing,
 * software distributed under the License is distributed on an
 * "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
 * KIND, either express or implied.  See the License for the
 * specific language governing permissions and limitations
 * under the License.
 */
// Auto-generated by Causeway-Stuff code generator.
package dita.globodiet.params.supplement;

import io.github.causewaystuff.companion.applib.jpa.EnumConverter;
import io.github.causewaystuff.companion.applib.jpa.EnumWithCode;
import io.github.causewaystuff.companion.applib.jpa.Persistable;
import io.github.causewaystuff.companion.applib.services.lookup.Cloneable;
import io.github.causewaystuff.companion.applib.services.search.SearchService;
import jakarta.inject.Inject;
import jakarta.inject.Named;
import jakarta.persistence.Column;
import jakarta.persistence.Convert;
import jakarta.persistence.Entity;
import jakarta.persistence.GeneratedValue;
import jakarta.persistence.GenerationType;
import jakarta.persistence.Id;
import jakarta.persistence.Table;
import jakarta.persistence.Transient;
import java.lang.Integer;
import java.lang.Override;
import java.lang.String;
import java.util.List;
import javax.annotation.processing.Generated;
import lombok.AllArgsConstructor;
import lombok.Getter;
import lombok.RequiredArgsConstructor;
import lombok.Setter;
import lombok.experimental.Accessors;
import org.apache.causeway.applib.ViewModel;
import org.apache.causeway.applib.annotation.Collection;
import org.apache.causeway.applib.annotation.DomainObject;
import org.apache.causeway.applib.annotation.DomainObjectLayout;
import org.apache.causeway.applib.annotation.Editing;
import org.apache.causeway.applib.annotation.Nature;
import org.apache.causeway.applib.annotation.Navigable;
import org.apache.causeway.applib.annotation.ObjectSupport;
import org.apache.causeway.applib.annotation.Optionality;
import org.apache.causeway.applib.annotation.Programmatic;
import org.apache.causeway.applib.annotation.Property;
import org.apache.causeway.applib.annotation.PropertyLayout;
import org.apache.causeway.applib.annotation.Snapshot;
import org.apache.causeway.applib.annotation.Where;
import org.apache.causeway.applib.services.repository.RepositoryService;

/**
 * Dietary supplement descriptor
 */
@Generated("io.github.causewaystuff.companion.codegen.domgen._GenEntity")
@Named("dita.globodiet.params.supplement.DietarySupplementDescriptor")
@DomainObject
@DomainObjectLayout(
        describedAs = "Dietary supplement descriptor",
        cssClassFa = "solid tablets .supplement-color,\n"
                + "solid tag .supplement-color .ov-size-80 .ov-right-55 .ov-bottom-55\n"
)
@Entity
@Table(
        name = "DS_DESCFACE"
)
public class DietarySupplementDescriptor implements Persistable, Cloneable<DietarySupplementDescriptor> {
    @Inject
    @Transient
    RepositoryService repositoryService;

    @Inject
    @Transient
    SearchService searchService;

    @Id
    @GeneratedValue(
            strategy = GenerationType.IDENTITY
    )
    private long id;

    /**
     * Descriptor code
     */
    @Property(
            optionality = Optionality.MANDATORY,
            editing = Editing.ENABLED
    )
    @PropertyLayout(
            fieldSetId = "details",
            sequence = "1",
            describedAs = "Descriptor code"
    )
    @Column(
            name = "\"DSDESCR_CODE\"",
            nullable = false,
            length = 2
    )
    @Getter
    @Setter
    private String code;

    /**
     * Descriptor name
     */
    @Property(
            optionality = Optionality.MANDATORY,
            editing = Editing.ENABLED
    )
    @PropertyLayout(
            fieldSetId = "details",
            sequence = "2",
            describedAs = "Descriptor name"
    )
    @Column(
            name = "\"DSDESCR_NAME\"",
            nullable = false,
            length = 100
    )
    @Getter
    @Setter
    private String name;

    /**
     * Facet code
     */
    @Property(
            optionality = Optionality.MANDATORY
    )
    @PropertyLayout(
            fieldSetId = "foreign",
            sequence = "3",
            describedAs = "Facet code",
            hidden = Where.ALL_TABLES
    )
    @Column(
            name = "\"DSFACET_CODE\"",
            nullable = false,
            length = 2
    )
    @Getter
    @Setter
    private String facetCode;

    /**
     * Only for the facet with Dsfacet_type=1,
     * for the supplement quantification If HHM=1 Then HHM method is proposed Else No HHM=0
     */
    @Property(
            optionality = Optionality.MANDATORY,
            editing = Editing.ENABLED
    )
    @PropertyLayout(
            fieldSetId = "details",
            sequence = "4",
            describedAs = "Only for the facet with Dsfacet_type=1,\n"
                    + "for the supplement quantification If HHM=1 Then HHM method is proposed Else No HHM=0"
    )
    @Column(
            name = "\"DSDESCR_HHM\"",
            nullable = false
    )
    @Getter
    @Setter
    private int householdMeasuresProposedQ;

    /**
     * Default Descriptor.
     * When this facet is displayed, the cursor has to be focussed on the default descriptor (only 1 defaulty):
     * 1=default,
     * 0=other
     */
    @Property(
            optionality = Optionality.MANDATORY,
            editing = Editing.ENABLED
    )
    @PropertyLayout(
            fieldSetId = "details",
            sequence = "5",
            describedAs = "Default Descriptor.\n"
                    + "When this facet is displayed, the cursor has to be focussed on the default descriptor (only 1 defaulty):\n"
                    + "1=default,\n"
                    + "0=other"
    )
    @Column(
            name = "\"DSDESCR_DEFAULT\"",
            nullable = false
    )
    @Getter
    @Setter
    @Convert(
            converter = DefaultDescriptor.Converter.class
    )
    private DefaultDescriptor defaultDescriptor;

    /**
     * Descriptor with type='other' : 1=yes 0=no
     */
    @Property(
            optionality = Optionality.MANDATORY,
            editing = Editing.ENABLED
    )
    @PropertyLayout(
            fieldSetId = "details",
            sequence = "6",
            describedAs = "Descriptor with type='other' : 1=yes 0=no"
    )
    @Column(
            name = "\"DSDESCR_OTHER\"",
            nullable = false
    )
    @Getter
    @Setter
    @Convert(
            converter = OtherQ.Converter.class
    )
    private OtherQ otherQ;

    /**
     * 0=not single descriptor 1=single descriptor
     */
    @Property(
            optionality = Optionality.MANDATORY,
            editing = Editing.ENABLED
    )
    @PropertyLayout(
            fieldSetId = "details",
            sequence = "7",
            describedAs = "0=not single descriptor 1=single descriptor"
    )
    @Column(
            name = "\"DSDESCR_SINGLE\"",
            nullable = false
    )
    @Getter
    @Setter
    @Convert(
            converter = SingleDescriptorQ.Converter.class
    )
    private SingleDescriptorQ singleDescriptorQ;

    /**
     * Display order (1=first, 2=second, …)
     */
    @Property(
            optionality = Optionality.MANDATORY,
            editing = Editing.ENABLED
    )
    @PropertyLayout(
            fieldSetId = "details",
            sequence = "8",
            describedAs = "Display order (1=first, 2=second, …)"
    )
    @Column(
            name = "\"DSDESCR_ORDER\"",
            nullable = false
    )
    @Getter
    @Setter
    private int displayOrder;

    /**
     * Not in name flag
     */
    @Property(
            optionality = Optionality.MANDATORY,
            editing = Editing.ENABLED
    )
    @PropertyLayout(
            fieldSetId = "details",
            sequence = "9",
            describedAs = "Not in name flag"
    )
    @Column(
            name = "\"NOTINNAME\"",
            nullable = false
    )
    @Getter
    @Setter
    private int notInName;

    @ObjectSupport
    public String title() {
        return String.format("%s (code=%s)", name, code);
    }

    @Override
    public String toString() {
        return "DietarySupplementDescriptor(" + "code=" + getCode() + ","
         +"name=" + getName() + ","
         +"facetCode=" + getFacetCode() + ","
         +"householdMeasuresProposedQ=" + getHouseholdMeasuresProposedQ() + ","
         +"defaultDescriptor=" + getDefaultDescriptor() + ","
         +"otherQ=" + getOtherQ() + ","
         +"singleDescriptorQ=" + getSingleDescriptorQ() + ","
         +"displayOrder=" + getDisplayOrder() + ","
         +"notInName=" + getNotInName() + ")";
    }

    @Programmatic
    @Override
    public DietarySupplementDescriptor copy() {
        var copy = repositoryService.detachedEntity(new DietarySupplementDescriptor());
        copy.setCode(getCode());
        copy.setName(getName());
        copy.setFacetCode(getFacetCode());
        copy.setHouseholdMeasuresProposedQ(getHouseholdMeasuresProposedQ());
        copy.setDefaultDescriptor(getDefaultDescriptor());
        copy.setOtherQ(getOtherQ());
        copy.setSingleDescriptorQ(getSingleDescriptorQ());
        copy.setDisplayOrder(getDisplayOrder());
        copy.setNotInName(getNotInName());
        return copy;
    }

    @Property(
            snapshot = Snapshot.EXCLUDED
    )
    @PropertyLayout(
            hidden = Where.EVERYWHERE,
            navigable = Navigable.PARENT
    )
    @Transient
    public DietarySupplementDescriptor.Manager getNavigableParent() {
        return new DietarySupplementDescriptor.Manager(searchService, "");
    }

    @Getter
    @Accessors(
            fluent = true
    )
    @RequiredArgsConstructor
    public enum DefaultDescriptor implements EnumWithCode<Integer> {

        /**
         * no description
         */
        DEFAULT(1, "default"),
        /**
         * no description
         */
        OTHER(0, "other");

        private final Integer code;

        private final String title;

        @jakarta.persistence.Converter
        public static final class Converter implements EnumConverter<DefaultDescriptor, Integer> {
            @Override
            public DefaultDescriptor[] values() {
                return DefaultDescriptor.values();
            }
        }
    }

    @Getter
    @Accessors(
            fluent = true
    )
    @RequiredArgsConstructor
    public enum OtherQ implements EnumWithCode<Integer> {

        /**
         * no description
         */
        NO(0, "no"),
        /**
         * no description
         */
        YES(1, "yes");

        private final Integer code;

        private final String title;

        @jakarta.persistence.Converter
        public static final class Converter implements EnumConverter<OtherQ, Integer> {
            @Override
            public OtherQ[] values() {
                return OtherQ.values();
            }
        }
    }

    @Getter
    @Accessors(
            fluent = true
    )
    @RequiredArgsConstructor
    public enum SingleDescriptorQ implements EnumWithCode<Integer> {

        /**
         * no description
         */
        NOT_SINGLE_DESCRIPTOR(0, "not single descriptor"),
        /**
         * no description
         */
        SINGLE_DESCRIPTOR(1, "single descriptor");

        private final Integer code;

        private final String title;

        @jakarta.persistence.Converter
        public static final class Converter implements EnumConverter<SingleDescriptorQ, Integer> {
            @Override
            public SingleDescriptorQ[] values() {
                return SingleDescriptorQ.values();
            }
        }
    }

    /**
     * Manager Viewmodel for @{link DietarySupplementDescriptor}
     */
    @Generated("io.github.causewaystuff.companion.codegen.domgen._GenEntity_Manager")
    @Named("dita.globodiet.params.supplement.DietarySupplementDescriptor.Manager")
    @DomainObject(
            nature = Nature.VIEW_MODEL
    )
    @DomainObjectLayout(
            describedAs = "Dietary supplement descriptor",
            cssClassFa = "solid tablets .supplement-color,\n"
                    + "solid tag .supplement-color .ov-size-80 .ov-right-55 .ov-bottom-55\n"
    )
    @AllArgsConstructor
    public static final class Manager implements ViewModel {
        public final SearchService searchService;

        @Property(
                optionality = Optionality.OPTIONAL,
                editing = Editing.ENABLED
        )
        @PropertyLayout(
                fieldSetId = "searchBar"
        )
        @Getter
        @Setter
        private String search;

        @ObjectSupport
        public String title() {
            return "Manage Dietary Supplement Descriptor";
        }

        @Collection
        public final List<DietarySupplementDescriptor> getListOfDietarySupplementDescriptor() {
            return searchService.search(DietarySupplementDescriptor.class, DietarySupplementDescriptor::title, search);
        }

        @Override
        public final String viewModelMemento() {
            return getSearch();
        }
    }
}
